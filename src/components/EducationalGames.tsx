...
function MemoryMatrixPairsGame({ difficulty, onFinished, opponentScore, synthSound, useFormulas }: MemoryProps) {
  ...
  // Watch matchesCount for win condition to avoid nested state updates inside callbacks
  useEffect(() => {
    if (cards.length === 0) return;
    if (matchesCount > 0 && matchesCount >= target) {
      const calculatedAcc = Math.round((target / Math.max(1, moves)) * 100);
      onFinished(score + 50, target, calculatedAcc);
    }
  }, [matchesCount, target, moves, score, onFinished, cards.length]);
