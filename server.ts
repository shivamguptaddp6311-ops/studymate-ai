...
app.post("/api/gemini/voice-script", async (req, res) => {
  try {
    ...
    Style Guidelines:
    - Language: Write in the requested language (${language || "detected language"}). If English, Hindi, or Hinglish, write exactly in that script/accent. If Hinglish, use Latin characters representing Hindi conversational slang (e.g., "Aap is calculation ko simplify karne ke liye...").
    - Start with a brief direct answer.
    - Explain concepts step by step.
    - Use simple, warm, teacher-like language first, then add more detail if appropriate.
    - Give examples when useful.
    - Summarize key points at the end.
    
    Depth Level:
    - "short": Highly brief, concise, directly answering with 1-2 sentences of explanation. Max 60-80 words.
    - "standard": Standard classroom explanation, friendly, clear. About 120-180 words.
    - "detailed": Multi-step detailed explanation, covering context, examples, details, and equations. About 250-400 words.
    
    Equation Guidelines:
    - Explain every calculation in order.
    - Clearly write out operators in spoken word (e.g., write "plus" instead of "+", "divided by" instead of "/", "multiplied by" instead of "*", "equals" instead of "=").
    - Read fractions, powers, roots, and equations naturally (e.g., "three fourths", "x squared", "the square root of nine").
    - Do NOT include complex markdown symbols, raw LaTeX, or code backticks inside the spoken parts because TTS reads them literally. Use clean text.
    ...
    if (!scriptText) {
      throw new Error("No script received from model.");
    }
